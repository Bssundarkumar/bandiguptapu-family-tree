(()=>{
  ...
  const SOURCE_PEOPLE=[
    ...
    {id:'subhash-child-1',fullName:'Sreshta',surname:'Bandiguptapu',gender:'Female',generationHint:7,parents:['venkata-siva-subhash','gummalla-jhansi-veera-durga-padma-jyothi'],spouses:[],children:[],confidence:'confirmed',notes:'Name confirmed directly by family.',sourceNote:'Family-confirmed correction to the inherited genealogy mapping.'},
  ...
  // Family-confirmed corrections always override old placeholders/cached values.
  const AUTHORITATIVE={
    'subhash-child-1':{fullName:'Sreshta',surname:'Bandiguptapu',gender:'Female',confidence:'confirmed',notes:'Name confirmed directly by family.',sourceNote:'Family-confirmed correction.'},
    'subhash-child-2':{fullName:'Varnisha',surname:'Bandiguptapu',gender:'Female',confidence:'confirmed',notes:'Name confirmed directly by family.',sourceNote:'Family-confirmed correction.'}
  };
  ...
  function ensureSourcePeople(){
    ...
    for(const [id,patch] of Object.entries(AUTHORITATIVE)){
      const p=byId(id);if(p)Object.assign(p,patch);
    }
  }
  ...
  function repairRelationships(persist=true){
    ensureSourcePeople();
    let repairs=0;const ids=new Set(people.map(p=>p.id));
    ...
    for(const p of people){
      ...
      if(p.confidence==='familyrecord'&&String(p.id).startsWith('hist-')&&!hasRel)p.sourceRelationshipStatus='unmapped-from-pdf';
      else if(hasRel)p.sourceRelationshipStatus='mapped';
    }
    if(persist){try{sset(KEY,JSON.stringify({version:VERSION,updatedAt:new Date().toISOString(),people,relationshipIntegrity:true,sourceMerge:true}))}catch(e){}}
    return repairs;
  }
  ...
  window.derivedSiblingIds=function(person){
    const p=typeof person==='string'?byId(person):person;if(!p)return[];const out=new Set();
    for(const pid of relationshipIds(p,'parents')){const parent=byId(pid);for(const cid of relationshipIds(parent,'children'))if(cid!==p.id&&exists(cid))out.add(cid)}
    return[...out];
  };
  ...
  if(typeof renderSide==='function'){
    ...
    renderSide=function(){const p=byId(current);if(!p)return originalRenderSide();const raw={parents:p.parents,children:p.children,spouses:p.spouses};p.parents=relationshipIds(p,'parents');p.children=relationshipIds(p,'children');p.spouses=relationshipIds(p,'spouses');try{return originalRenderSide()}finally{p.parents=raw.parents;p.children=raw.children;p.spouses=raw.spouses}};window.renderSide=renderSide;
  }
  if(typeof save==='function'){const originalSave=save;save=function(){repairRelationships(false);return originalSave()};window.save=save;}
  if(typeof render==='function'){const originalRender=render;render=function(){repairRelationships(false);return originalRender()};window.render=render;}

  const badge=document.createElement('div');badge.id='integrityBadge';badge.textContent='✓ Family links synced';badge.title=`Relationship engine v${VERSION}${startupRepairs?` · ${startupRepairs} links repaired`:''}`;badge.style.cssText='position:fixed;right:12px;bottom:calc(76px + env(safe-area-inset-bottom));z-index:40;padding:7px 10px;border-radius:999px;background:rgba(13,24,37,.9);border:1px solid rgba(106,219,167,.28);color:#8fe0b7;font:700 11px system-ui,-apple-system,sans-serif;backdrop-filter:blur(14px);pointer-events:none;opacity:.82;transition:opacity .5s';document.body.appendChild(badge);setTimeout(()=>badge.style.opacity='.28',3200);
